...
void loop() {
    if (millis() > start_time + freq){
      if (!no_line_detected()){
        moveTrack();
        if (isObstacleDetected(20)){
          turn90();
          delay(1200);
          do{
            circle();
...
bool isObstacleDetected(long trigger_distance_to_object){
  long distance_to_object = readPing();
  if(distance_to_object < trigger_distance_to_object){
...
void circle(){
  ...
  leftMotor1.setSpeed(55);
  leftMotor2.setSpeed(55);
  rightMotor1.setSpeed(250);
  rightMotor2.setSpeed(250);
}
...
void turn90(){
  ...
  leftMotor1.setSpeed(100);
  leftMotor2.setSpeed(100);
  rightMotor1.setSpeed(100);
  rightMotor2.setSpeed(100);
}
...
void moveTrack(void){
  ...
  if ((num2 == 0) && (num1 == 0) && (num3 == 0)) {
    ...
    leftMotor1.run(3); rightMotor1.run(3);//3-> left
    leftMotor2.run(3); rightMotor2.run(3);
    leftMotor1.setSpeed(0); rightMotor1.setSpeed(100);
    leftMotor2.setSpeed(0); rightMotor2.setSpeed(100);
    while (1) {
      if ((Serial.available() > 0))
      {
        isTrackMode = false;
    ...
    leftMotor1.run(4); rightMotor1.run(4);//4-> right
    leftMotor2.run(4); rightMotor2.run(4);
    leftMotor1.setSpeed(100); rightMotor1.setSpeed(0);
    leftMotor2.setSpeed(100); rightMotor2.setSpeed(0);
    while (1) {
      if ((Serial.available() > 0))
      {
        isTrackMode = false;
